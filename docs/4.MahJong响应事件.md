...
|cards|当前牌|int[]|是|
|actions|动作集|ActionOption[]|是| ActionOption：[{"action":0,"options":[[2,3,4],[55.56,57],[25.26,27]]}]
|weaveItems|组合集|WeaveItemResponse[][]|是| [chair][{"weaveType":1,"centerCard":21,"open":false,"provider":"提供者"}]
|discardCount|出牌总数|int|是|
|discardCards|出牌的记录|int|是|
|totalNumber|总局数|int|是|
|currentNumber|当前局数|int|是|
|scores|分数|int[]|是|
|heroes|玩家信息|Map<int,Hero>|是| Hero：{"userId":"用户ID","userName":"昵称","gender":"性别","avatar":"头像","tableNo":"桌子编号","circleId":"联盟编号"}
